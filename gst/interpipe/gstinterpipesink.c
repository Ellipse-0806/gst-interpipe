...
static GstStateChangeReturn gst_inter_pipe_sink_change_state(GstElement *element, GstStateChange transition);
...
G_DEFINE_TYPE_WITH_CODE(GstInterPipeSink, gst_inter_pipe_sink,
                        GST_TYPE_APP_SINK, G_IMPLEMENT_INTERFACE(GST_INTER_PIPE_TYPE_INODE, gst_inter_pipe_inode_init));
...
gst_inter_pipe_sink_class_init(GstInterPipeSinkClass *klass)
{
  ...
  element_class->change_state = gst_inter_pipe_sink_change_state;
}
...
gst_inter_pipe_sink_set_caps(GstBaseSink *base, GstCaps *caps)
{
  ...
  g_mutex_lock(&sink->listeners_mutex);
  if (sink->caps_negotiated && (gst_caps_can_intersect(sink->caps_negotiated, caps)))
  {
    gpointer data[2];
...
gst_inter_pipe_sink_event(GstBaseSink *base, GstEvent *event)
{
  ...
  g_mutex_unlock(&sink->listeners_mutex);
  return GST_BASE_SINK_CLASS(gst_inter_pipe_sink_parent_class)->event(base, event);
}
...
GType listener_signal_get_type(void)
{
  static GType type = 0;
  if (g_once_init_enter(&type))
  {
    static const GEnumValue values[] = {
        {LISTENER_ADDED, "Added", "added"},
        {LISTENER_REMOVED, "Removed", "removed"},
        {0, NULL, NULL}};
    GType tmp = g_enum_register_static("ListenerSignal", values);
    g_once_init_leave(&type, tmp);
  }
  return type;
}

static void post_listeners_change(GstInterPipeSink *sink, ListenerSignal signal, guint num_listeners, const gchar *listener_name)
{
  if (!sink->bus)
  {
    sink->bus = gst_element_get_bus(GST_ELEMENT(sink));
  ...
  GstMessage *msg = gst_message_new_element(GST_OBJECT_CAST(sink),
                                            gst_structure_new("GstInterPipeSinkListeners",
                                                              "event", listener_signal_get_type(), signal,
                                                              "listener_name", G_TYPE_STRING, listener_name,
                                                              "num_listeners", G_TYPE_UINT, num_listeners,
                                                              NULL));

  gst_bus_post(sink->bus, msg);
}
...
static GstStateChangeReturn gst_inter_pipe_sink_change_state(GstElement *element, GstStateChange transition)
{
  ...
  // 既定の動作を実行
  ret = GST_ELEMENT_CLASS(gst_inter_pipe_sink_parent_class)->change_state(element, transition);
                                 ...
                                 GstInterPipeIListener *listener)
{
  ...
  if (src_negotiated)
  {
    ...
    if (!sink->caps_negotiated && !has_listeners && !gst_caps_is_equal(srccaps, sinkcaps))
    {
...
add_to_list:
  ...
  guint num_listeners = g_hash_table_size(listeners);
  g_mutex_unlock(&sink->listeners_mutex);
  post_listeners_change(sink, LISTENER_ADDED, num_listeners, listener_name);
                                    ...
                                    GstInterPipeIListener *listener)
{
  ...
  guint num_listeners = g_hash_table_size(listeners);
  g_mutex_unlock(&sink->listeners_mutex);
  post_listeners_change(sink, LISTENER_REMOVED, num_listeners, listener_name);
